-- B1 strict publish gates (additive): placeholder OCR, provisional scoring,
-- and Hören content_verified audio checks. Classic A1 exams without these
-- metadata flags remain unaffected.
...
BEGIN
  ...
  LOOP
    ...
    -- Audio Hören content verification — only when media is present and status exists.
    ...
    -- Provisional scoring (B1 OCR drafts).
